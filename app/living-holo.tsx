...
export function LivingHolo(){
  ...
  return <main className="living-site" style={{'--neural-color':current.color} as React.CSSProperties}>
    ...
    <div hidden={view!=='culture'}><Culture mind={{name:"Holo",text:liveEntries[0]?.text??"…"}} neural={{energy:current.energy,curiosity:current.curiosity,name:current.name}} body={live.status==='live'?live.body:null} drive={live.status==='live'?live.drive:null} brainOnline={brainOnline} active={view==='culture'} onRecord={()=>{}} onOpenRecords={()=>openRecords()} onOpenNeeds={()=>{setView('nectar');window.scrollTo({top:0});}}/></div>
